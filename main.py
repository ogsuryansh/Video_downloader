...
async def send_large_video(message: types.Message, filename: str):
    ...
    - Rejects and deletes videos larger than 100 MB.
    - Sends as document if >50 MB but <100 MB.
    - Sends as video if <50 MB.
    - Deletes the local video file immediately after sending or rejection.
    - Schedules deletion of the sent message after 30 minutes.
    """
    try:
        file_size = os.path.getsize(filename)

        # ✅ 1. Check size limit: 100 MB
        if file_size > 100 * 1024 * 1024:
            os.remove(filename)
            logging.info(f"File too large (>100MB), deleted: {filename}")
            await message.answer("❌ The downloaded video is larger than 100 MB and can't be sent. Please try a smaller video.")
            return None

        # ✅ 2. Send as document if >50 MB
        if file_size > 50 * 1024 * 1024:
            ...
            # ✅ 3. Send as video if ≤50 MB
            sent_message = await message.answer_video(
                FSInputFile(filename),
                caption="🎥 Here is your video.\n\n🚀 *This message will be deleted in 30 minutes to prevent copyright issues.*"
            )

        # ✅ 4. Delete file after sending
        os.remove(filename)
        logging.info(f"Deleted file after sending: {filename}")

        # ✅ 5. Schedule auto-deletion of message
